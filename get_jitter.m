...
jit_abs = zeros(1,length(inds));  % Inicialização vetor de jit. abs.
jit_rel = zeros(1,length(inds));  % Inicialização vetor de jit. rel.
shim_abs = zeros(1,length(inds)); % Inicialização vetor de shi. abs.
shim_rel = zeros(1,length(inds)); % Inicialização vetor de shi. rel.
amp_x = [];
amp_std = [];
...
for i = 1:length(inds) % Laço for para cada janela específica.
    ...
    [amp_max,loc_max] = findpeaks(filt,'MinPeakHeight',mx); % Obtém as posições dos picos.
