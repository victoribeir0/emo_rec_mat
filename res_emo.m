% Determina a matriz de resultados.
...
for emo = 1:length(emos)
    ...
    for n = 1:N
        ...
        if emos(emo) == em
            ...
            if strcmp(ext, 'f0')
                [~, m_f0, nrg] = get_global(dados.data,idx,dados.fs);
                [int_voz, ~] = get_times(nrg,1);
                
                y = [sum(int_voz); mean(int_voz); m_f0];
            end
...
function [MFCC,idx] = process(dados,rnn,Sf)
